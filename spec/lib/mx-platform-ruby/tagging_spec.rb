...
RSpec.describe ::MxPlatformRuby::Tagging do
  ...
  let(:tagging_response) { { 'tagging' => tagging_attributes } }
  ...
  before { allow(::MxPlatformRuby.client).to receive(:make_request).and_return(taggings_response) }

  describe 'create_tagging' do
    before { allow(::MxPlatformRuby.client).to receive(:make_request).and_return(tagging_response) }
