...
module MxPlatformRuby
  class ProcessorAccountNumberBodyAccountNumbersInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MxPlatformRuby::ProcessorAccountNumberBodyAccountNumbersInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MxPlatformRuby::ProcessorAccountNumberBodyAccountNumbersInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_guid, member_guid, user_guid, account_number, guid, institution_number, loan_guarantor, loan_reference_number, passed_validation, routing_number, sequence_number, transit_number].hash
    end
